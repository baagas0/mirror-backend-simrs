...
class Controller {
    static async syncWithAssesmenIgd(req, res) {
      ...
      const { registrasi_id, cppt_id } = req.body;

      try {
        const retrieveResep = await resepRnap.findOne({where:{registrasi_id, cppt_id}})
        const data_cppt = await cppt.findOne({where:{id: cppt_id}})
        console.log(data_cppt)

        let asesmen = {};

        // CEK DATA ASSESMEN
        if (data_cppt && data_cppt.dataValues && data_cppt.dataValues.asesmen && data_cppt.dataValues.asesmen.planning && data_cppt.dataValues.asesmen.planning.obat_jadi && data_cppt.dataValues.asesmen.planning.obat_racikan) {
          asesmen = data_cppt.dataValues.asesmen
        } else {
          res.status(500).json({ status: 500, message: "Data obat tidak ditemukan pada cppt ini.", data: [] })
          return "Data obat tidak ditemukan pada cppt ini."
        }

        if(!retrieveResep){
          const id=uuid_v4();
          const createResep = await resepRnap.create({id,registrasi_id,cppt_id,createdBy:req.dataUsers.id},{transaction:t})
          let dataBulkRacik = [];
          let dataBulkDetail = [];
          for(let item of asesmen.planning.obat_racikan){
            ...
            for(let itemm of item.obat_jadi){
              dataBulkDetail.push({
                id:uuid_v4(),
                resep_id:id,
                resep_racik_rnap_id:idResepRacik,
          ...
          for(let item of asesmen.planning.obat_jadi){
            dataBulkDetail.push({
              id:uuid_v4(),
              resep_id:id,
          ...
          await resepRacikRnap.bulkCreate(dataBulkRacik,{transaction:t})
          await resepDetailRnap.bulkCreate(dataBulkDetail,{transaction:t})
        } else {
		...
		static register(req, res) {
				resepRnap.create({ id: uuid_v4(), ...req.body }).then(hasil => {
		...
		static update(req, res) {
				resepRnap.update({ ...req.body}, { where: { id:req.body.id } }).then(hasil => {
		...
		static async list(req, res) {
				...
				try {
						let data = await sq.query(`select rr.id as id_resep_rnap, * from resep_rnap rr
		...
		static delete(req, res) {
				const { id } = req.body
				resepRnap.destroy({ where: { id } }).then(hasil => {
						res.status(200).json({ status: 200, message: "sukses" })
				}).catch(error => {
						console.log(error);
						res.status(500).json({ status: 500, message: "gagal", data: error })
    ...
    static async listWithParam(req, res) {
      const { registrasi_id, cppt_id, tahap_resep } = req.body;
      let params = {};
      if (registrasi_id) params.registrasi_id = registrasi_id;
      if (tahap_resep) params.tahap_resep = tahap_resep;
      if (cppt_id) params.cppt_id = cppt_id;
      resepRnap.findAll({ where: params }).then(hasil => {
        res.status(200).json({
          status: 200,
          message: "sukses",
          data: hasil
    ...
    static async getWithDetailResep(req, res) {
      ...
      if (id) params.id = id;
      try {
        const data = await resepRnap.findAll({
          where: params,
          include: [{
            model: registrasi,
            as: 'registrasi'
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          const detail = await sq.query(`select * from resep_detail_rnap where resep_id='${element.id}'`, s);
