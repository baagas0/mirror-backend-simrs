...
class Controller {
  static async register(req, res) {
    const {
      ...
      harga_barang_custom: harga_satuan_custom,
      harga_pokok,
      keterangan,
    ...
    try {
      let cekPenjualan = await penjualan.findAll({
        where: { id: penjualan_id },
      });
      if (cekPenjualan.length == 0) {
        res
          .status(201)
          .json({ status: 204, message: "penjualan_id tidak ada" });
      } else {
        if (cekPenjualan[0].status_penjualan != 1) {
          res
            .status(201)
            .json({ status: 204, message: "status penjualan bukan 1" });
        } else {
          let cekPenjualanOperasi = await penjualanOperasi.findAll({
            where: { penjualan_id, ms_barang_id },
          });
          if (cekPenjualanOperasi.length > 0) {
            ...
            let barang = await kurangiStock({
              ms_gudang_id,
              isi: `'${ms_barang_id}'`,
              bulk_barang: [{ ms_barang_id, penjualan_id, qty_barang: qty }],
            });
            if (barang.cekHasil.length > 0) {
              ...
              let hasil = await sq.transaction(async (t) => {
                ...
                await penjualan.update(
                  { harga_total_operasi, discount, tax, total_penjualan },
                  { where: { id: penjualan_id }, transaction: t },
                );
                await stockBarang.bulkCreate(barang.stock, {
              ...
              res
                .status(200)
                .json({ status: 200, message: "sukses", data: hasil });
            }
          }
        }
      }
  ...
  static async update(req, res) {
    ...
    try {
      let dataOperasi = await sq.query(
        `select po.*,p.ms_gudang_id from penjualan_operasi po join penjualan p on p.id = po.penjualan_id where po."deletedAt" isnull and po.id = '${id}'`,
        s,
      );
      if (dataOperasi[0].status_penjualan_operasi != 1) {
        res
          .status(201)
          .json({ status: 204, message: "status penjualan operasi bukan 1" });
      } else {
        let stock = [];
        let hisInv = [];
        let barang = [];
        ...
        if (dataOperasi[0].qty != qty) {
          ...
          let data = await updateStock({
            ms_gudang_id: dataOperasi[0].ms_gudang_id,
            bulk_barang: dataOperasi,
            isi: `'${dataOperasi[0].ms_barang_id}'`,
          });
        ...
        if (barang.length > 0) {
          res
            .status(201)
            .json({ status: 204, message: "stock tidak cukup", data: barang });
        } else {
          await sq.transaction(async (t) => {
            await penjualanOperasi.update(
              {
                qty,
                harga_satuan,
                harga_satuan_custom,
            ...
            await stockBarang.bulkCreate(stock, {
              updateOnDuplicate: ["qty"],
              transaction: t,
            });
  ...
  static async list(req, res) {
    try {
      let data = await sq.query(
        `select po.id as penjualan_operasi_id, po.*,
            ...
            where po."deletedAt" isnull order by po."createdAt" desc`,
        s,
      );
    ...
    try {
      let data = await sq.query(
        `select po.id as penjualan_operasi_id, po.*,
            ...
            order by p.kode_penjualan, mb.nama_barang`,
        s,
      );
    ...
    try {
      let data = await sq.query(
        `select po.id as penjualan_operasi_id, po.*,
            ...
            join ms_satuan_barang msb on msb.id = mb.ms_satuan_barang_id
            join penjualan p on p.id = po.penjualan_id
            where po."deletedAt" isnull and po.id = '${id}'`,
        s,
      );
